...
source(here("CSIPDA_helper_functions.R")) #read in analysis functions from other R document
...
FLAG_below_droplet_threshold <-well_dat_and_meta %>%
  ...
  select(Well, SampleID, Assay, `Accepted Droplets`, Run)%>% #Run refers to the date of the experiment 
  unique()
...
merged_ref_targets <- merged_ref_targets %>%
  ...
  #make RPP30 early data the estimate for cell quant. 
...
#generate list of replicates that are >3fold higher. If they are >10fold higher, they will be removed from analyses. We run additional replicates if one is >3fold different. 
