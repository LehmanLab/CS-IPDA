...
library(here)
# dicatates that directory starts at CS-IPDA
here() #check that directory is set...

# read in helper functions 
source(here("CSIPDA_helper_functions.R")) #read in analysis functions from other R document ***its important this file lives under the start of the here set above***


#### 2) EXPERIMENT DETAILS ####    

# DATE OF ANALYSIS:  
# WHAT IS BEING ANALYZED: ACH2 cells containing provirus at serial dilutions 
EXPERIMENT_NAME <- "ACH2_CSIPDA_mock"

## QUANTASOFT EXPORTED DATA LOCATION
QS_EXPORTED_DATA_LOCATION <- here()


# OUTPUT DATA PATH 
OUTPUT_FILE_PATH <- here()

#add in dilution factor for RPP30, DSI cutoff for samples analyzed via RPP30 assay 
...
####1) OPH Reservoir Master Inventory_unmerged maintains experimental dates and storage information for all samples. This is the "backbone" file we're wanting to modify with this merge ##### 

# telling R where to find and export files 
detach("package:here", unload=TRUE) #reset here
setwd("/Volumes/fh/fast/overbaugh_j/grp/JOLabShared/CarolynFish/Projects/Reservoir ddPCR/OPH") #show R where you want your project to start 
library(here) #re-run here and it should start at the right place 

normalized_single_triple_total<- read_csv(here("OPH_reservoir/OPH pipeline data/M42-5 and M96-9/OPH_M45_99_normalized_single_triple_total_2021-09-14.csv"))

unmerged_master <- read_csv(here("OPH_reservoir/OPHsample_track.csv")) %>%
  rename(short_id= oph_shortID)

#read in all pull lists and rename the variables we NEED in Reservoir Master Inventory--> aka all pull list variables 

pull1<-read_csv(here("OPH_sample_select/pull_lists/OPH Reservoir Sample Pull 1.csv")) %>%
  select(ophinfptid,
         shortOPHid,
         time_hidt_days,
         monthn90d,
         ...
         box,
         rowpos) %>%
  rename(inf_id = "ophinfptid",
         short_id = "shortOPHid", 
         time_hidt = "time_hidt_days", 
         month = "monthn90d")%>%
  mutate(ophxinfptid="")
...
pull2<-read_csv(here("OPH_sample_select/pull_lists/pull list2.csv")) %>%
  select(2:5,12:16) %>%
  rename(inf_id = "OPH inf ID",
         short_id = "Short OPH ID", 
         time_hidt = "time_hidt_days", 
         month = "Month",
         freezer= "PBMC freezer (pre-pull)",
         rack= "PBMCs rack (pre-pull)",
         box= "PBMCs box (pre-pull)",
         rowpos= "PBMCs position # (pre-pull)", 
...
pull11<-read_csv(here
                 ("OPH_sample_select/pull_lists/20200831_OPH_Reservoir_TestList_Inventory_PullList11.csv")) %>%
  select(-column, -nth_vial, -enrollcohort, -origptid_nvpexpose, -`Pull box`, -visitcode, -indata_reslist, -...18, -...19)%>%
  rename(inf_id = "ophinfptid",
         short_id = "short_OPH", 
         time_hidt = "time_hidt_days", 
         month = "monthn90d") %>%
  mutate(sampletypecode= "311")

colnames(pull11)

pull3v24 <-read_csv(here("OPH_sample_select/pull_lists/OPH_Reservoir_TestList_Inventory_PullList03_24.csv")) %>%
  select(-col_pre, -nth_vial, -enrollcohort, -origptid_nvpexpose, -pull_notes, -visitcode, -indata_reslist, -pos_post) %>%
...
pull12_M24 <-read_csv(here("OPH_sample_select/pull_lists/OPH_Reservoir_TestList_Inventory_PullList12_M24only.csv")) %>%
  select(-column, -nth_vial, -enrollcohort, -origptid_nvpexpose, -`Pull position`, -visitcode, -indata_reslist)%>% 
  rename(inf_id = "ophinfptid",
         time_hidt = "time_hidt_days", 
         month = "monthn90d"
  ) %>%
...
#pull lists containing M42/45 and M96/99 data are: 5, 6 and the remainder of 12
here()
pull5<- read_csv("/Volumes/grp/JOLabShared/CarolynFish/Projects/Reservoir ddPCR/OPH/OPH_sample_select/pull_lists/OPH_Reservoir_test  list_inventory Pull list05.csv")%>%
  select(-`OPH 5-Pull box 1`)

pull6<- read_csv("/Volumes/grp/JOLabShared/CarolynFish/Projects/Reservoir ddPCR/OPH/OPH_sample_select/pull_lists/OPH_Reservoir_test  list_inventory Pull list06.csv")%>%
  select(-`OPH 5-Pull box 1`) 

pull12_M42_96<- read_csv("/Volumes/grp/JOLabShared/CarolynFish/Projects/Reservoir ddPCR/OPH/OPH_sample_select/pull_lists/OPH_Reservoir_TestList_Inventory_PullList12_M42_96.csv")%>%
  select(-`Pull position`)

#THIS PULL LIST HAS NOT YET GOTTEN THE GREEN LIGHT FOR TESTING. it contains non M24 samples from pull list 12. 
#pull13_M42_96 <- read_csv("OPH_sample_select/pull_lists/OPH_Reservoir_TestList_Inventory_PullList13_M42_96.csv")

#there are 2 back up samples that were pulled and denoted by a 0.1 added to short_id
pull_lists<- rbind(pull5, pull6, pull12_M42_96 )



#add in full oph_id for merging 
short_id <- pull_lists %>%
  select(short_id, ophinfptid, ophxinfptid, origptid_nvpexpose) %>%
  unique()

fullOPHid_master <- unmerged_master %>%
  merge(., short_id, by= "short_id")%>%
  rename(monthn90d= "month")%>%
  select(-23,-24)




#2) S. Benki has requested that this Reservoir Master maintain ALL pull list variables from now on- 
#So, I will merge in the pull list information for lists. Then, I will just manually add in when new pull lists need to be added 

colnames(fullOPHid_master)

colnames(pull_lists)

MERGED_master <- fullOPHid_master %>%
  merge(., pull_lists, by = c("sampleid","monthn90d", "short_id", "ophinfptid", "ophxinfptid", "origptid_nvpexpose"), all = TRUE) %>%
  rename(
    ext_group= "extraction_group",
    ext_date = "extraction_date",
    num_aliquots = "aliquots_extracted",
    RPP30_done = "RPP30 complete",
    vol_RPP30 = "uL used RPP30",
    use_other = "other uses for samples",
    vol_other = "uL template used other",
    dna_box = "storage_location",
    dna_pos = "storage_pos",
    frzr_pre= "freezer",
    rack_pre= "rack",
    box_pre= "box",
...
colnames(MERGED_master)



#3) After Reservoir Master Inventory has all pull list info, I will merge in assay data and change variable naming to match the NC_Benki_OPH Reservoir Master Inventory

#reading in QSFT data files to merge into Master Inventory.
#we definitely need dsi, cells, tp copies per uL, intact HIV DNA per 1e6 cells, intact HIV DNA per 1e6 T cells, total HIV DNA per 1e6 cells, total HIV DNA per 1e6 T cells 

all_HIVassay_rep_count<- all_HIVassay_rep_count %>%
  rename(hiv_reps = "reps",
         sampleid= "SampleID") 
all_HIVassay_rep_count%>%select(-Assay)

all_RPP30_rep_count <- all_RPP30_rep_count %>%
  rename(rpp30_reps = "reps", 
         sampleid= "SampleID")%>% 
  select(-Assay)

colnames(assay)

assay <- normalized_single_triple_total %>%
  filter(Target == "triple positive")%>%
  
  select(-Assay, 
         -Target, 
         -Neg_for_cluster, 
         -`Copies per uL in cluster`,
         -`Total HIV copies per uL in merged well`,
         -`Cluster percent of total HIV`,
         -reps,
         -copies_per_1E6_cells,
         -copies_per_1E6_T_cells,
         -`Accepted Droplets`, 
  ...
  rename(sampleid = "SampleID",
         tp_droplets = "Merged droplets in cluster",
         total_droplets = "Total droplets in merged wells",
         total_hivdnat = "total_HIV_copies_per_1E6_T_cells",
         total_hivdna = "total_HIV_copies_per_1E6_cells",
         pbmc_ul = "cells_per_uL_template",
         tcell_ul = "T_cells_per_uL_template",
         dsi = "DSI",
         intact_hivdnat = "DSI_corrected_per_1E6_T_cells",
         intact_hivdna = "DSI_corrected_per_1E6_cells",
         dna_sheared= "high_shearing",
         pbmcs_int= "total_cell_interrogated",
         tcell_int= "total_T_cells_interrogated",
         tp_neg= "triple_pos_neg",
         min_pbmcs_int= "interrogated_1e5_cells"
  )  %>%
  merge(., all_HIVassay_rep_count, by= "sampleid", all=TRUE) %>%
  merge(., all_RPP30_rep_count, by= "sampleid", all=TRUE)

colnames(assay)

#merging in inventory and assay data
merged_master_data <- MERGED_master %>%
  merge(., assay, by = c("sampleid"), all = TRUE)

merged_master_data$vol_other[is.na(merged_master_data$vol_other)] = 0

merged_master_data <- merged_master_data %>%
  mutate(dna_vol= 50* as.numeric(num_aliquots),
         dna_left= dna_vol-((2*rpp30_reps)+(4*hiv_reps)+vol_other)) %>%
  select(-vol_RPP30) %>%
  rename(
    intact_neg= "tp_neg",
    tot_pbmcs= "pbmcs_int",
    tot_tcells= "tcell_int",
    min_pbmcs= "min_pbmcs_int"
  ) %>%
  mutate(percent_sheared= 100-as.numeric(percent_intact)) %>%
  unique()


colnames(merged_master_data)

merged_master_for_epi <- merged_master_data 


save_path <- here("OPH_decay_CF_20210304.csv")
save_path2 <- here("Data/OPH_Reservoir_pull_info.csv")

save_path3 <- here("OPH_decay_CF_20210310.2.csv")

write_csv(merged_master_data, path = save_path)

write_csv(merged_master_for_epi, path = save_path3)