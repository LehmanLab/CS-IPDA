...
#Make dilution factor variable for RPP30 samples 
RPP30_dilution <- 20


#Set a percent intact cutoff for sample QC. Usually I omit intact HIV data from samples with >40% sheared DNA. 
DSI_CUTOFF <- 60
...
# CSIPDA target names (historically referred to as Assay2 from IPDA adaptation)
...
#for the sample that uses Jurkat/Jlat cell mixes only (Jurkat are tetraploid)
JURKAT_REF_GENES_PER_GENOME <- 4

## data frame containing assays, dyenames and target names

ASSAY_TARGET_DYE <- data.frame(Assay = c(rep(ASSAY2_NAME, times = ASSAY2_NUM_TARGETS),
                                         rep(ASSAY3_NAME, times = RPP30_deltaD_NUM_TARGETS)),
                               Target = c(ASSAY2_TARGET_1, ASSAY2_TARGET_2, ASSAY2_TARGET_3,
                                          RPP30_deltaD_TARGET1, RPP30_deltaD_TARGET2, RPP30_deltaD_TARGET3),
                               `DyeName(s)` = c(ASSAY2_DYE_1, ASSAY2_DYE_2, ASSAY2_DYE_3,
...
add_meta_data <- function(no_meta_df, plate_layout_path){
  ...
  no_meta_df %>%
    ...
    mutate(Assay = replace(Assay, Assay == "Assay2", "CSIPDA"))
...
file_paths_to_meta_wells <- function(ExperimentName) {
  ...
  well_dat_and_meta_for_merge$Target <- toupper(well_dat_and_meta_for_merge$Target)
...
file_paths_to_meta_clusters <- function(ExperimentName) {
  ...
  well_dat_and_meta_for_merge <- read_csv(WELL_DAT_FILE_PATH,locale = readr:: locale(encoding = "latin1")) %>%
    add_plate(PLATE_LAYOUT_FILE_PATH, well_ids_column = "Well") %>%
    mutate(Assay = replace(as.character(Assay), Assay == "Assay2", "CSIPDA"))%>%
...
merge_in_possible <- function(cluster_dat_df, possible_clusters_df){
  ...
  cluster_dat_df %>%
    ...
    mutate(`Ch2 StdDev` = as.numeric(str_replace(`Ch2 StdDev`, ",", ""))) %>%
